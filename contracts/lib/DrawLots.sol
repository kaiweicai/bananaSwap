...
contract DrawLots is Initializable, IDrawLots {
    // Random number generate instance
    IRandomNumber public random;
    ...
    function initialize(IRandomNumber _random) public initializer {
        random = _random;
    }
    ...
    function getValidWinningTail(uint _salt, uint _i, uint _totalQuantity) private returns (uint) {
        ...
        while (flag) {
            flag = false;
            randomNumber = random.randomNumber(_salt);
